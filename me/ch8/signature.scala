...
def f14[A](x: List[A], f: A => Boolean): Boolean = x.exists(f) || x.forall(f)
...
def f20[A](x: Set[A], f: A => Boolean): Boolean = x.forall(f) || x.exists(f)
...
def f26[A](x: Option[A], f: A => Boolean): Boolean = x.forall(f) || x.exists(f)
...
def f36[A, B](x: Either[A, B], f: B => Boolean): Boolean = x.forall(f) || x.exists(f)
